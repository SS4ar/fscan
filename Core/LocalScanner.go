...
func (s *LocalScanStrategy) Execute(info Common.HostInfo, ch *chan struct{}, wg *sync.WaitGroup) {
	Common.LogBase("Executing local information collection")
...
func (s *LocalScanStrategy) LogPluginInfo() {
	...
	if isCustomMode {
		Common.LogBase(fmt.Sprintf("Local mode: Using specified plugins: %s", strings.Join(allPlugins, ", ")))
		return
	}
	...
	if len(applicablePlugins) > 0 {
		Common.LogBase(fmt.Sprintf("Local mode: Using local plugins: %s", strings.Join(applicablePlugins, ", ")))
	} else {
