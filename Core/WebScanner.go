...
func (s *WebScanStrategy) Execute(info Common.HostInfo, ch *chan struct{}, wg *sync.WaitGroup) {
	Common.LogBase("Starting web scan")
...
func (s *WebScanStrategy) LogPluginInfo() {
	...
	if isCustomMode {
		Common.LogBase(fmt.Sprintf("Web scan mode: Using specified plugins: %s", strings.Join(allPlugins, ", ")))
		return
	}
	...
	if len(applicablePlugins) > 0 {
		Common.LogBase(fmt.Sprintf("Web scan mode: Using web plugins: %s", strings.Join(applicablePlugins, ", ")))
	} else {
		Common.LogBase("Web scan mode: No available web plugins found")
	}
}
